...
安装去官网：https://docs.docker.com/engine/install/centos/
...
docker run -p 9000:9000 -p 9001:9001 --name minio \
  ...
  minio/minio:RELEASE.2021-07-22T05-23-32Z server /data --console-address ":9001"
```
...
COPY docker-test.jar /docker/docker-test.jar
ENTRYPOINT ["java", "-Xmx256m", "-jar", "/docker/docker-test.jar"]
```
...
2. `MAINTAINER xxx xxxxx@qq.com`指定镜像的作者及联系方式；

3. `COPY docker-test.jar /docker/docker-test.jar`表示将当前目录下的docker-test.jar拷贝到`openjdk:8u212-jre`镜像里的/docker目录下，名称也为docker-test.jar；

4. `ENTRYPOINT ["java", "-Xmx256m", "-jar", "/docker/docker-test.jar"]`表示运行`java -jar`运行镜像里的jar包，JVM内存最大分配为256m（因为要运行的微服务较多并且虚拟机内存只有6GB，所以内存分配小了点，实际可以根据宿主服务器的配置做出相应调整）。

##### 2.构建镜像

DockerFile创建好后，在当前目录下运行以下命令构建镜像：
